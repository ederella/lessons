...
public class Any extends General {
	...
	@Override
	<A extends General, B extends General> A assignment_attempt(A target, B source) {
	...
	@Override
	public Any sum(General g) {
		Any result = new Any();
		Any value = assignment_attempt(result, g);
		if(value == null)
			return this;
		result.setId(this.id + value.id);
		result.setAuthor(this.author + value.author);
		result.setStatus(this.status + value.status);
